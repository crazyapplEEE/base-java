 ...
 * @Version:V1.0
 */
@Slf4j @Service public class SysBaseApiImpl implements ISysBaseAPI {
    /**
     * 当前系统数据库类型
     */
    ...
    @Resource private SysAnnouncementMapper sysAnnouncementMapper;
    @Resource private SysAnnouncementSendMapper sysAnnouncementSendMapper;
    @Resource private SysRoleMapper roleMapper;
    @Resource private SysDepartMapper departMapper;
    @Resource private SysCategoryMapper categoryMapper;

    @Autowired private ISysDataSourceService dataSourceService;
    @Autowired private ISysUserDepartService sysUserDepartService;
    ...
    @Autowired private ISysTokenLoginidService sysTokenLoginidService;
    @Autowired private ISysLoginidRolesService sysLoginidRolesService;
    @Autowired private ISysTokenUserService sysTokenUserService;

    @Override @Cacheable(cacheNames = CacheConstant.SYS_USERS_CACHE, key = "#username")
    ...
    @Override public String translateDictFromTable(String table, String text, String code, String key) {
        return sysDictService.queryTableDictTextByKey(table, text, code, key);
    }

    @Override public String translateDict(String code, String key) {
        return sysDictService.queryDictTextByKey(code, key);
    }

    @Override public List<SysPermissionDataRuleModel> queryPermissionDataRule(String component, String requestPath,
        String username) {
        ...
        if (oConvertUtils.isNotEmpty(component)) {
            ...
            if (currentSyspermission == null || currentSyspermission.size() == 0) {
                ...
                if (regUrl != null) {
                    currentSyspermission = sysPermissionMapper.selectList(
                        new LambdaQueryWrapper<SysPermission>().eq(SysPermission::getMenuType, 2)
                            .eq(SysPermission::getUrl, regUrl).eq(SysPermission::getDelFlag, 0));
                }
            }
        }
        if (currentSyspermission != null && currentSyspermission.size() > 0) {
            ...
            for (SysPermission sysPermission : currentSyspermission) {
                ...
                List<SysPermissionDataRule> temp =
                    sysPermissionDataRuleService.queryPermissionDataRules(username, sysPermission.getId());
    ...
    @Override public SysUserCacheInfo getCacheUser(String username) {
    ...
    @Override public List<String> getRolesByUsername(String username) {
        return sysUserRoleMapper.getRoleByUserName(username);
    }

    @Override public List<String> getDepartIdsByUsername(String username) {
    ...
    @Override public List<String> getDepartNamesByUsername(String username) {
        List<SysDepart> list = sysDepartService.queryDepartsByUsername(username);
        List<String> result = new ArrayList<>(list.size());
        for (SysDepart depart : list) {
    ...
    @Override public DictModel getParentDepartId(String departId) {
        SysDepart depart = departMapper.getParentDepartId(departId);
        DictModel model = new DictModel(depart.getId(), depart.getParentId());
        return model;
    }

    @Override @Cacheable(value = CacheConstant.SYS_DICT_CACHE, key = "#code", unless = "#result == null ")
    public List<DictModel> queryDictItemsByCode(String code) {
        return sysDictService.queryDictItemsByCode(code);
    }

    @Override public List<DictModel> queryTableDictItemsByCode(String table, String text, String code) {
    ...
    @Override public String queryLoginidByToken(String token) {
        return sysTokenLoginidService.queryLoginidByToken(token);
    }

    @Override public String queryRolesByLoginid(String loginid) {
        return sysLoginidRolesService.queryRolesByLoginid(loginid);
    }

    @Override public SysTokenUser queryUserByToken(String token) {
        return sysTokenUserService.queryByToken(token);
    }

    @Override public List<DictModel> queryAllDepartBackDictModel() {
        return sysDictService.queryAllDepartBackDictModel();
    }

    @Override public void sendSysAnnouncement(MessageDTO message) {
...
//        }
    ...
    @Override public void sendBusAnnouncement(BusMessageDTO message) {
...
//        }
    ...
    @Override public void sendTemplateAnnouncement(TemplateMessageDTO message) {
...
//        }
    ...
    @Override public void sendBusTemplateAnnouncement(BusTemplateMessageDTO message) {
        String templateCode = message.getTemplateCode();
        String title = message.getTitle();
        Map<String, String> map = message.getTemplateParam();
        String fromUser = message.getFromUser();
...
//        }
    ...
    @Override public String parseTemplateByCode(TemplateDTO templateDTO) {
...
//        return content;
    ...
    @Override public void updateSysAnnounReadFlag(String busType, String busId) {
        SysAnnouncement announcement = sysAnnouncementMapper.selectOne(
            new QueryWrapper<SysAnnouncement>().eq("bus_type", busType).eq("bus_id", busId));
    ...
    @Override public List<DictModel> queryAllDict() {
        // 查询并排序
    ...
    @Override public List<SysCategoryModel> queryAllDSysCategory() {
    ...
    @Override public List<String> queryTableDictByKeys(String table, String text, String code, String[] keyArray) {
        return sysDictService.queryTableDictByKeys(table, text, code, Joiner.on(",").join(keyArray));
    }

    @Override public List<ComboModel> queryAllUserBackCombo() {
    ...
    @Override public JSONObject queryAllUser(String userIds, Integer pageNo, Integer pageSize) {
    ...
    @Override public List<ComboModel> queryAllRole() {
        List<ComboModel> list = new ArrayList<ComboModel>();
    ...
    @Override public List<ComboModel> queryAllRole(String[] roleIds) {
        List<ComboModel> list = new ArrayList<ComboModel>();
        List<SysRole> roleList = roleMapper.selectList(new QueryWrapper<SysRole>());
        for (SysRole role : roleList) {
            ComboModel model = new ComboModel();
    ...
    @Override public List<String> getRoleIdsByUsername(String username) {
        return sysUserRoleMapper.getRoleIdByUserName(username);
    }

    @Override public String getDepartIdsByOrgCode(String orgCode) {
        return departMapper.queryDepartIdByOrgCode(orgCode);
    }

    @Override public List<SysDepartModel> getAllSysDepart() {
    ...
    @Override public DynamicDataSourceModel getDynamicDbSourceById(String dbSourceId) {
    ...
    @Override public DynamicDataSourceModel getDynamicDbSourceByCode(String dbSourceCode) {
    ...
    @Override public List<String> getDeptHeadByDepId(String deptId) {
        List<SysUser> userList = userMapper.selectList(
            new QueryWrapper<SysUser>().like("depart_ids", deptId).eq("status", 1).eq("del_flag", 0));
    ...
    @Override public void sendWebSocketMsg(String[] userIds, String cmd) {
        JSONObject obj = new JSONObject();
        obj.put(WebsocketConst.MSG_CMD, cmd);
//        webSocket.sendMessage(userIds, obj.toJSONString());
    }

    @Override public List<LoginUser> queryAllUserByIds(String[] userIds) {
        QueryWrapper<SysUser> queryWrapper = new QueryWrapper<SysUser>().eq("status", 1).eq("del_flag", 0);
     ...
     * @param userId
     */
    @Override public void meetingSignWebsocket(String userId) {
        JSONObject obj = new JSONObject();
...
//        webSocket.sendMessage(obj.toJSONString());
    ...
    @Override public List<LoginUser> queryUserByNames(String[] userNames) {
        QueryWrapper<SysUser> queryWrapper = new QueryWrapper<SysUser>().eq("status", 1).eq("del_flag", 0);
    ...
    @Override public SysDepartModel selectAllById(String id) {
    ...
    @Override public List<String> queryDeptUsersByUserId(String userId) {
        List<String> userIds = new ArrayList<>();
        List<SysUserDepart> userDepartList =
            sysUserDepartService.list(new QueryWrapper<SysUserDepart>().eq("user_id", userId));
        if (userDepartList != null) {
            ...
            if (oConvertUtils.isNotEmpty(orgCodes)) {
                ...
                List<SysUserDepart> userList =
                    sysUserDepartService.list(new QueryWrapper<SysUserDepart>().in("dep_id", listIds));
     ...
     * @return
     */
    @Override public Set<String> getUserRoleSet(String username) {
        // 查询用户拥有的角色集合
        List<String> roles = sysUserRoleMapper.getRoleByUserName(username);
        //        log.info("-------通过数据库读取用户拥有的角色Roles------ username: " + username + ", roles size: " + (roles == null ? 0 :
        //            roles.size()));
     ...
     * @return
     */
    @Override public Set<String> getUserPermissionSet(String username) {
     ...
     * @return
     */
    @Override public boolean hasOnlineAuth(OnlineAuthDTO onlineAuthDTO) {
     ...
     * @return
     */
    @Override public Set<String> queryUserRoles(String username) {
     ...
     * @return
     */
    @Override public Set<String> queryUserAuths(String username) {
     ...
     * @return
     */
    @Override public List<JSONObject> queryUsersByUsernames(String usernames) {
    ...
    @Override public List<JSONObject> queryUsersByIds(String ids) {
        LambdaQueryWrapper<SysUser> queryWrapper = new LambdaQueryWrapper<>();
     ...
     * @return
     */
    @Override public List<JSONObject> queryDepartsByOrgcodes(String orgCodes) {
    ...
    @Override public List<JSONObject> queryDepartsByIds(String ids) {
        LambdaQueryWrapper<SysDepart> queryWrapper = new LambdaQueryWrapper<>();
...
//                webSocket.sendMessage(sysUser.getId(), obj.toJSONString());
     ...
     * @param content
     */
    @Override public void sendEmailMsg(String email, String title, String content) {
...
//        emailHandle.SendMsg(email, title, content);
     ...
     * @return
     */
    @Override public List<Map> getDeptUserByOrgCode(String orgCode) {
        //1.获取公司信息
    ...
    @Override public List<String> getRoleList() {
        final Set<String> roleSet = getRoleSet();
        return new ArrayList<>(roleSet);
    }

    @Override public Set<String> getRoleSet() {
        ...
        // 加上jeecg本身的role
        Set<String> roleSet = queryUserRoles(user.getUsername());
