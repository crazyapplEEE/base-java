 ...
 * @Version: V1.0
 */
@RestController @RequestMapping("/sys/category") @Slf4j public class SysCategoryController {
     ...
     * @return
     */
    @GetMapping(value = "/rootList") public Result<IPage<SysCategory>> queryPageList(SysCategory sysCategory,
     ...
     * @return
     */
    @PostMapping(value = "/add") public Result<SysCategory> add(@RequestBody SysCategory sysCategory) {
     ...
     * @return
     */
    @PostMapping(value = "/edit") public Result<SysCategory> edit(@RequestBody SysCategory sysCategory) {
        Result<SysCategory> result = new Result<SysCategory>();
     ...
     * @return
     */
    @PostMapping(value = "/delete") public Result<SysCategory> delete(@RequestParam(name = "id") String id) {
        Result<SysCategory> result = new Result<SysCategory>();
     ...
     * @return
     */
    @PostMapping(value = "/deleteBatch") public Result<SysCategory> deleteBatch(
     ...
     * @return
     */
    @GetMapping(value = "/queryById") public Result<SysCategory> queryById(@RequestParam(name = "id") String id) {
        Result<SysCategory> result = new Result<SysCategory>();
        SysCategory sysCategory = sysCategoryService.getById(id);
        if (sysCategory == null) {
            result.error500("未找到对应实体");
    ...
    @RequestMapping(value = "/exportXls") public ModelAndView exportXls(HttpServletRequest request,
        ...
        QueryWrapper<SysCategory> queryWrapper =
            QueryGenerator.initQueryWrapper(sysCategory, request.getParameterMap());
    ...
    @PostMapping(value = "/importExcel") public Result<?> importExcel(HttpServletRequest request,
        ...
        for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
            ...
            try {
                List<SysCategory> listSysCategorys =
                    ExcelImportUtil.importExcel(file.getInputStream(), SysCategory.class, params);
                //按照编码长度排序
     ...
     * 加载单个数据 用于回显
     */
    @GetMapping(value = "/loadOne") public Result<SysCategory> loadOne(@RequestParam(name = "field") String field,
     ...
     * 加载节点的子数据
     */
    @GetMapping(value = "/loadTreeChildren") public Result<List<TreeSelectModel>> loadTreeChildren(
     ...
     * 加载一级节点/如果是同步 则所有数据
     */
    @GetMapping(value = "/loadTreeRoot") public Result<List<TreeSelectModel>> loadTreeRoot(
        @RequestParam(name = "async") Boolean async, @RequestParam(name = "pcode") String pcode) {
        Result<List<TreeSelectModel>> result = new Result<>();
        try {
     ...
     * @return
     */
    @GetMapping(value = "/checkCode") public Result<?> checkCode(
     ...
     * @return
     */
    @GetMapping(value = "/loadTreeData") public Result<List<TreeSelectModel>> loadDict(
        @RequestParam(name = "pid", required = false) String pid,
     ...
     * @return
     */
    @GetMapping(value = "/loadDictItem") public Result<List<String>> loadDictItem(
        @RequestParam(name = "ids") String ids,
    ...
    @GetMapping(value = "/loadDictItemByCodes")
    public Result<List<String>> loadDictItemByCodes(@RequestParam(name = "codes") String codes) {
        Result<List<String>> result = new Result<>();
        // 非空判断
        if (StringUtils.isBlank(codes)) {
            result.setSuccess(false);
            result.setMessage("codes 不能为空");
            return result;
        }
        // 查询数据
        List<String> textList = sysCategoryService.loadDictItemByCodes(codes);
        result.setSuccess(true);
        result.setResult(textList);
        return result;
    }
     ...
     * @return
     */
    @GetMapping(value = "/loadAllData") public Result<List<DictModel>> loadAllData(
