...
@Mapper public interface SysPermissionMapper extends BaseMapper<SysPermission> {
     ...
     * 修改菜单状态字段： 是否子节点
     */
    @Update("update sys_permission set is_leaf=#{leaf} where id = #{id}") int setMenuLeaf(@Param("id") String id,
        @Param("leaf") int leaf);
    ...
    List<SysPermission> queryByRoleCodes(@Param("roleCodes") List<String> roleCodes);
}
