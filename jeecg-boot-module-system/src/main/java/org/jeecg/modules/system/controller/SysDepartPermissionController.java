 ...
 * @Version: V1.0
 */
@Slf4j @Api(tags = "JEECG-部门权限表") @RestController @RequestMapping("/sys/sysDepartPermission")
public class SysDepartPermissionController extends JeecgController<SysDepartPermission, ISysDepartPermissionService> {
     ...
     * @return
     */
    @ApiOperation(value = "部门权限表-分页列表查询", notes = "部门权限表-分页列表查询") @GetMapping(value = "/list")
    public Result<?> queryPageList(SysDepartPermission sysDepartPermission,
        @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
        @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize, HttpServletRequest req) {
        QueryWrapper<SysDepartPermission> queryWrapper =
            QueryGenerator.initQueryWrapper(sysDepartPermission, req.getParameterMap());
     ...
     * @return
     */
    @ApiOperation(value = "部门权限表-添加", notes = "部门权限表-添加") @PostMapping(value = "/add") public Result<?> add(
        @RequestBody SysDepartPermission sysDepartPermission) {
     ...
     * @return
     */
    @ApiOperation(value = "部门权限表-编辑", notes = "部门权限表-编辑") @PostMapping(value = "/edit") public Result<?> edit(
        @RequestBody SysDepartPermission sysDepartPermission) {
     ...
     * @return
     */
    @ApiOperation(value = "部门权限表-通过id删除", notes = "部门权限表-通过id删除") @PostMapping(value = "/delete")
     ...
     * @return
     */
    @ApiOperation(value = "部门权限表-批量删除", notes = "部门权限表-批量删除") @PostMapping(value = "/deleteBatch")
     ...
     * @return
     */
    @ApiOperation(value = "部门权限表-通过id查询", notes = "部门权限表-通过id查询") @GetMapping(value = "/queryById")
    ...
    @RequestMapping(value = "/exportXls") public ModelAndView exportXls(HttpServletRequest request,
        SysDepartPermission sysDepartPermission) {
     ...
     * 部门管理授权查询数据规则数据
     */
    @GetMapping(value = "/datarule/{permissionId}/{departId}") public Result<?> loadDatarule(
        ...
        if (list == null || list.size() == 0) {
            ...
            LambdaQueryWrapper<SysDepartPermission> query =
                new LambdaQueryWrapper<SysDepartPermission>().eq(SysDepartPermission::getPermissionId, permissionId)
     ...
     * @return
     */
    @GetMapping(value = "/queryDeptRolePermission") public Result<List<String>> queryDeptRolePermission(
        @RequestParam(name = "roleId") String roleId) {
        Result<List<String>> result = new Result<>();
        try {
            List<SysDepartRolePermission> list = sysDepartRolePermissionService.list(
                new QueryWrapper<SysDepartRolePermission>().lambda().eq(SysDepartRolePermission::getRoleId, roleId));
            result.setResult(
                list.stream().map(SysDepartRolePermission -> String.valueOf(SysDepartRolePermission.getPermissionId()))
     ...
     * @return
     */
    @PostMapping(value = "/saveDeptRolePermission") public Result<String> saveDeptRolePermission(
     ...
     * @return
     */
    @GetMapping(value = "/queryTreeListForDeptRole") public Result<Map<String, Object>> queryTreeListForDeptRole(
        @RequestParam(name = "departId") String departId, HttpServletRequest request) {
        ...
        try {
            ...
            query.inSql(SysPermission::getId,
