 ...
 * @Version: V1.0
 */
@Service public class SysCategoryServiceImpl extends ServiceImpl<SysCategoryMapper, SysCategory>
    implements ISysCategoryService {

    @Override public void addSysCategory(SysCategory sysCategory) {
        String categoryCode = "";
    ...
    @Override public void updateSysCategory(SysCategory sysCategory) {
    ...
    @Override public List<TreeSelectModel> queryListByCode(String pcode) throws JeecgBootException {
        String pid = ROOT_PID_VALUE;
        if (oConvertUtils.isNotEmpty(pcode)) {
            List<SysCategory> list =
                baseMapper.selectList(new LambdaQueryWrapper<SysCategory>().eq(SysCategory::getCode, pcode));
    ...
    @Override public List<TreeSelectModel> queryListByPid(String pid) {
    ...
    @Override public List<TreeSelectModel> queryListByPid(String pid, Map<String, String> condition) {
        if (oConvertUtils.isEmpty(pid)) {
            pid = ROOT_PID_VALUE;
        }
    ...
    @Override public String queryIdByCode(String code) {
        return baseMapper.queryIdByCode(code);
    }

    @Override @Transactional(rollbackFor = Exception.class) public void deleteSysCategory(String ids) {
        ...
        if (oConvertUtils.isNotEmpty(pids)) {
            LambdaUpdateWrapper<SysCategory> updateWrapper =
                new UpdateWrapper<SysCategory>().lambda().in(SysCategory::getId, Arrays.asList(pids.split(",")))
    ...
    private String queryTreePids(String ids) {
        ...
        for (String id : idArr) {
            if (id != null) {
                ...
                if ((dataList == null || dataList.size() == 0) && !Arrays.asList(idArr).contains(metaPid) && !sb
                    .toString().contains(metaPid)) {
    ...
    @Override public List<String> loadDictItem(String ids) {
        return this.loadDictItem(ids, true);
    }

    @Override public List<String> loadDictItem(String ids, boolean delNotExist) {
    ...
    @Override public List<String> loadDictItemByCodes(String codes) {
        String[] codeArray = codes.split(",");
        LambdaQueryWrapper<SysCategory> query = new LambdaQueryWrapper<>();
        query.in(SysCategory::getCode, Arrays.asList(codeArray));
        // 查询数据
        List<SysCategory> list = super.list(query);
        // 取出name并返回
        ...
        for (int i = 0; i < list.size(); ++i) {
            if (i == list.size() - 1) {
                idBuilder.append(list.get(i).getId());
            } else {
                idBuilder.append(list.get(i).getId()).append(",");
            }
        }

        return loadDictItem(idBuilder.toString());
    }
