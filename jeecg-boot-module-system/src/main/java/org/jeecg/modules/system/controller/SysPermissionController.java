 ...
 * @since 2018-12-21
 */
@Slf4j @RestController @RequestMapping("/sys/permission") @Api(tags = "JEECG-系统权限")
public class SysPermissionController {
     ...
     * @return
     */
    @GetMapping(value = "/list") public Result<List<SysPermissionTree>> list() {
     ...
     * @return
     */
    @GetMapping(value = "/getSystemMenuList") public Result<List<SysPermissionTree>> getSystemMenuList() {
        long start = System.currentTimeMillis();
        Result<List<SysPermissionTree>> result = new Result<>();
        try {
            LambdaQueryWrapper<SysPermission> query = new LambdaQueryWrapper<>();
     ...
     * @return
     */
    @GetMapping(value = "/getSystemSubmenu") public Result<List<SysPermissionTree>> getSystemSubmenu(
     ...
     */
    @GetMapping("/getSystemSubmenuBatch") public Result getSystemSubmenuBatch(
     ...
     * @return
     */
    @GetMapping(value = "/getUserPermissionByToken") public Result<?> getUserPermissionByToken() {
        ...
        try {
            ...
            if (!PermissionDataUtil.hasIndexPage(metaList)) {
                SysPermission indexMenu = sysPermissionService.list(
                    new LambdaQueryWrapper<SysPermission>().eq(SysPermission::getName, "首页")).get(0);
     ...
     */
    //@RequiresRoles({ "admin" })
    @PostMapping(value = "/add") public Result<SysPermission> add(@RequestBody SysPermission permission) {
     ...
     */
    //@RequiresRoles({ "admin" })
    @PostMapping(value = "/delete") public Result<SysPermission> delete(@RequestBody String idJson) {
        Result<SysPermission> result = new Result<>();
        try {
            final JSONObject jsonObject = JSON.parseObject(idJson);
     ...
     */
    //@RequiresRoles({ "admin" })
    @PostMapping(value = "/deleteBatch") public Result<SysPermission> deleteBatch(
     ...
     * @return
     */
    @GetMapping(value = "/queryTreeList") public Result<Map<String, Object>> queryTreeList() {
     ...
     * @return
     */
    @GetMapping(value = "/queryListAsync") public Result<List<TreeModel>> queryAsync(
     ...
     * @return
     */
    @GetMapping(value = "/queryRolePermission") public Result<List<String>> queryRolePermission(
     ...
     */
    //@RequiresRoles({ "admin" })
    @PostMapping(value = "/saveRolePermission") public Result<String> saveRolePermission(@RequestBody JSONObject json) {
        long start = System.currentTimeMillis();
    ...
    private void getAuthJsonArray(JSONArray jsonArray, List<SysPermission> metaList) {
        for (SysPermission permission : metaList) {
            ...
            JSONObject json = null;
            if (permission.getMenuType().equals(CommonConstant.MENU_TYPE_2) && CommonConstant.STATUS_1.equals(
    ...
    private void getPermissionJsonArray(JSONArray jsonArray, List<SysPermission> metaList, JSONObject parentJson) {
        for (SysPermission permission : metaList) {
            ...
            if (parentJson == null && oConvertUtils.isEmpty(tempPid)) {
            ...
            } else if (parentJson != null && oConvertUtils.isNotEmpty(tempPid) && tempPid.equals(
                parentJson.getString("id"))) {
                ...
                if (permission.getMenuType().equals(CommonConstant.MENU_TYPE_2)) {
                    ...
                    // 类型( 0：一级菜单 1：子菜单 2：按钮 )
                } else if (permission.getMenuType().equals(CommonConstant.MENU_TYPE_1) || permission.getMenuType()
                    .equals(CommonConstant.MENU_TYPE_0)) {
    ...
    private JSONObject getPermissionJsonObject(SysPermission permission) {
        ...
        if (permission.getMenuType().equals(CommonConstant.MENU_TYPE_2)) {
            ...
            //json.put("describe", permission.getName());
            return null;
        } else if (permission.getMenuType().equals(CommonConstant.MENU_TYPE_0) || permission.getMenuType()
            .equals(CommonConstant.MENU_TYPE_1)) {
    ...
    /**
     * 判断是否外网URL 例如： http://localhost:8080/jeecg-boot/swagger-ui.html#/ 支持特殊格式： {{ window._CONFIG['domianURL'] }}/druid/
    ...
    /**
     * 通过URL生成路由name（去掉URL前缀斜杠，替换内容中的斜杠‘/’为-） 举例： URL = /isystem/role RouteName = isystem-role
     *
     * @return
     */
     ...
     * @return
     */
    @GetMapping(value = "/getPermRuleListByPermId") public Result<List<SysPermissionDataRule>> getPermRuleListByPermId(
        SysPermissionDataRule sysPermissionDataRule) {
        List<SysPermissionDataRule> permRuleList =
     ...
     */
    //@RequiresRoles({ "admin" })
    @PostMapping(value = "/addPermissionRule") public Result<SysPermissionDataRule> addPermissionRule(
        @RequestBody SysPermissionDataRule sysPermissionDataRule) {
     ...
     */
    //@RequiresRoles({ "admin" })
    @PostMapping(value = "/deletePermissionRule") public Result<SysPermissionDataRule> deletePermissionRule(
     ...
     * @return
     */
    @GetMapping(value = "/queryPermissionRule") public Result<List<SysPermissionDataRule>> queryPermissionRule(
        SysPermissionDataRule sysPermissionDataRule) {
        Result<List<SysPermissionDataRule>> result = new Result<>();
        try {
            List<SysPermissionDataRule> permRuleList =
                sysPermissionDataRuleService.queryPermissionRule(sysPermissionDataRule);
            result.setResult(permRuleList);
            result.success("查询成功！");
        } catch (Exception e) {
            log.error(e.getMessage(), e);
     ...
     * @return
     */
    @GetMapping(value = "/queryDepartPermission") public Result<List<String>> queryDepartPermission(
     ...
     */
    //@RequiresRoles({ "admin" })
    @PostMapping(value = "/saveDepartPermission") public Result<String> saveDepartPermission(
    ...
    @ApiOperation("根据角色集合（逗号隔开）获取权限集合") @GetMapping(value = "/queryPermissionByRoleCodes")
    public Result<?> queryPermissionByRoleCodes(@RequestParam("roleCodes") String roleCodes) {
        final List<String> roleCodeList = Arrays.asList(roleCodes.split(","));

        Result<JSONObject> result = new Result<>();
        List<SysPermission> metaList = sysPermissionService.queryByRoleCodes(roleCodeList);
